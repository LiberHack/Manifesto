...
const filteredTeams = computed(() => {
  ...
  if (selectedSkills.value.length)
    result = result.filter(t => selectedSkills.value.some(s => t.skills_wanted?.includes(s)))
  return result
})
</script>
...
<template>
  <main class="max-w-4xl mx-auto p-6 space-y-8">
    ...
    <div class="space-y-3">
      ...
      <div v-if="allSkills.length" class="flex flex-wrap gap-1.5">
        <button v-for="skill in allSkills" :key="skill"
          ...
          :class="selectedSkills.includes(skill) ? 'badge-primary' : 'opacity-60 hover:opacity-100'"
    ...
    <div v-if="!filteredTeams.length" class="text-center opacity-50 py-12 text-xl font-bold uppercase">
      No open teams yet.
    </div>

    <div class="grid gap-4 sm:grid-cols-2">
      <NuxtLink v-for="team in filteredTeams" :key="team.id" :to="`/ops/teams/${team.id}`"
