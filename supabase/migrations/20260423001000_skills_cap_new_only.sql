...
BEGIN
  ...
  IF (
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.skills WHERE lower(name) = lower(skill)
    )
  ) > 5 THEN
    RAISE EXCEPTION 'too_many_skills' USING HINT = 'A participant may not register more than 5 new skills';
  END IF;
