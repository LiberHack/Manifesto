...
export default defineEventHandler(async (event) => {
  ...
  let query = supabase
    .from('teams')
    .select('id, name, leader_id, skills_wanted, description, created_at, members:registrations(id)')
  ...
  if (error) throw createError({ statusCode: 500, message: 'Failed to fetch teams' })
